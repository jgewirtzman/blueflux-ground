# Complete LGR3 Flux Processing Workflow - Start to Finish
...
# Load LGR3 auxfile
#lgr3_auxfile <- read_csv("intermediate/auxfiles/tree_auxfile_lgr3_complete.csv") %>% mutate(start.time = as.POSIXct(start.time, tz = ifelse(lgr3_data_tz == "", "UTC", lgr3_data_tz)))
lgr3_auxfile <- read_csv("intermediate/auxfiles/tree_auxfile_all_instruments_additional.csv") %>% mutate(start.time = as.POSIXct(start.time, tz = ifelse(lgr3_data_tz == "", "UTC", lgr3_data_tz)))
...
for(batch_num in 1:num_batches) {
  ...
  manID_batch <- click.peak2(
    ...
    sleep = 3,
    plot.lim = c(200, 1200),
    save.plots = paste0("intermediate/lgr3_batch_", batch_num, "_plots")
  )
...
#write_csv(manID.lgr3, "intermediate/lgr3_manual_identification_results.csv")
write_csv(manID.lgr3, "intermediate/lgr3_manual_identification_results_additional.csv")
...
# Save flux results
write_csv(CO2_flux_lgr3, "intermediate/CO2_flux_lgr3_results_additional.csv")
if(exists("CH4_flux_lgr3")) {
  write_csv(CH4_flux_lgr3, "intermediate/CH4_flux_lgr3_results_additional.csv")
}
...
write_csv(CO2_best_lgr3, "intermediate/CO2_best_flux_lgr3_results_additional.csv")
if(exists("CH4_best_lgr3")) {
  write_csv(CH4_best_lgr3, "intermediate/CH4_best_flux_lgr3_results_additional.csv")
}
...
flux2pdf(
  ...
  outfile = "intermediate/LGR3_flux_plots_complete_additional.pdf",
  width = 11.6,
  height = 8.2
)

flux2pdf(
  plot.list = CO2_plots_lgr3,
  outfile = "intermediate/LGR3_CO2_flux_plots.pdf",
  width = 11.6,
  height = 8.2
)

if(exists("CH4_plots_lgr3")) {
  flux2pdf(
    plot.list = CH4_plots_lgr3,
    outfile = "intermediate/LGR3_CH4_flux_plots.pdf",
    width = 11.6,
    height = 8.2
  )
}
...
# flux2pdf(
#   plot.list = all_plots_lgr3,
#   outfile = "intermediate/LGR3_flux_plots_complete.pdf",
#   width = 11.6,
#   height = 8.2
# )
# 
...
write_csv(final_dataset, "intermediate/LGR3_final_complete_dataset_additional.csv")
...
cat("\nFiles created:\n")
cat("- intermediate/LGR3_final_complete_dataset.csv (complete final dataset)\n")
cat("- intermediate/CO2_best_flux_lgr3_results.csv (CO2 flux results)\n")
if(exists("CH4_best_lgr3")) {
  cat("- intermediate/CH4_best_flux_lgr3_results.csv (CH4 flux results)\n")
}
cat("- intermediate/LGR3_flux_plots_complete.pdf (all plots)\n")
