...
cat("\n=== STEP 6: STARTING PICARRO SOIL WATER PROCESSING ===\n")

# Check if we have Picarro data
if ("Picarro" %in% names(analyzer_data) && !is.null(analyzer_data[["Picarro"]])) {
  
  cat("Processing Picarro soil water data with", nrow(analyzer_data[["Picarro"]]), "rows\n")
  
  # Load the Picarro auxfile for soil water measurements
  picarro_auxfile_path <- "intermediate/auxfiles/soilwater_auxfile_picarro_goflux.csv"
  ...
  cat("Picarro soil water auxfile loaded:", nrow(picarro_auxfile_corrected), "measurements\n")
  ...
  # =============================================================================
  # STEP 6A: CREATE OBSERVATION WINDOWS FOR PICARRO SOIL WATER
  # =============================================================================
  
  cat("\n=== STEP 6A: CREATING OBSERVATION WINDOWS FOR PICARRO SOIL WATER ===\n")
  
  # Create observation windows for manual identification
  ow.picarro <- obs.win(
    inputfile = picarro_data_corrected,
    auxfile = picarro_auxfile_corrected,
    gastype = "CH4dry_ppb",      # Primary gas for soil water flux measurements
  ...
  if (length(good_windows) == 0) {
    cat("ERROR: No good observation windows found for Picarro soil water data\n")
    ...
    # =============================================================================
    # STEP 6B: MANUAL IDENTIFICATION FOR PICARRO SOIL WATER (IN BATCHES)
    # =============================================================================
    
    cat("\n=== STEP 6B: MANUAL IDENTIFICATION FOR PICARRO SOIL WATER ===\n")
    ...
    cat("Processing", total_measurements, "Picarro soil water measurements in", num_batches, "batches\n")
    ...
    for(batch_num in 1:num_batches) {
      ...
      cat("\n=== Processing Picarro Soil Water Batch", batch_num, "of", num_batches, "===\n")
      ...
      tryCatch({
        manID_batch <- click.peak2(
          ...
          save.plots = paste0("intermediate/picarro_soilwater_batch_", batch_num, "_plots"),
          warn.length = 60
        )
        ...
        cat("Error in Picarro soil water batch", batch_num, ":", e$message, "\n")
    ...
    if(length(successful_picarro_batches) > 0) {
      ...
      cat("\nPicarro soil water manual identification complete!\n")
      ...
      write_csv(manID.picarro, "intermediate/picarro_soilwater_manual_identification_CH4_results.csv")
      cat("Results saved to: intermediate/picarro_soilwater_manual_identification_CH4_results.csv\n")
      
      # =============================================================================
      # STEP 6C: FLUX CALCULATIONS FOR PICARRO SOIL WATER
      # =============================================================================
      ...
      if("CH4dry_ppb" %in% names(manID.picarro)) {
        cat("Calculating Picarro soil water CH4 fluxes...\n")
      ...
      if("CO2dry_ppm" %in% names(manID.picarro)) {
        cat("Calculating Picarro soil water CO2 fluxes...\n")
      ...
      if(!is.null(CH4_flux_picarro)) {
        write_csv(CH4_flux_picarro, "intermediate/CH4_flux_picarro_soilwater_results.csv")
        cat("Picarro soil water CH4 flux calculation complete:", nrow(CH4_flux_picarro), "measurements\n")
      }
      
      if(!is.null(CO2_flux_picarro)) {
        write_csv(CO2_flux_picarro, "intermediate/CO2_flux_picarro_soilwater_results.csv")
        cat("Picarro soil water CO2 flux calculation complete:", nrow(CO2_flux_picarro), "measurements\n")
      }
      
      # =============================================================================
      # STEP 6D: BEST FLUX ANALYSIS FOR PICARRO SOIL WATER
      # =============================================================================
      ...
      if(!is.null(CH4_flux_picarro)) {
        cat("Running best.flux analysis on Picarro soil water CH4 data...\n")
      ...
      if(!is.null(CO2_flux_picarro)) {
        cat("Running best.flux analysis on Picarro soil water CO2 data...\n")
      ...
      if(exists("CH4_best_picarro")) {
        write_csv(CH4_best_picarro, "intermediate/CH4_best_flux_picarro_soilwater_results.csv")
      }
      if(exists("CO2_best_picarro")) {
        write_csv(CO2_best_picarro, "intermediate/CO2_best_flux_picarro_soilwater_results.csv")
      }
      
      cat("Picarro soil water best flux analysis complete!\n")
      
      # =============================================================================
      # STEP 6E: QUALITY ASSESSMENT FOR PICARRO SOIL WATER
      # =============================================================================
      ...
      if(exists("CH4_best_picarro")) {
        ...
        cat("Picarro Soil Water CH4 Quality Distribution:\n")
      ...
      if(exists("CO2_best_picarro")) {
        ...
        cat("\nPicarro Soil Water CO2 Quality Distribution:\n")
        print(co2_quality_picarro)
      }
      
      cat("\nPicarro Soil Water Processing Summary:\n")
      ...
      # =============================================================================
      # STEP 6F: CREATE COMPREHENSIVE PLOTS FOR PICARRO SOIL WATER
      # =============================================================================
      
      cat("\n=== STEP 6F: CREATING COMPREHENSIVE PLOTS FOR PICARRO SOIL WATER ===\n")
      ...
      plot.list <- c()
      if(exists("CO2_plots")) plot.list <- c(plot.list, CO2_plots)
      if(exists("CH4_plots")) plot.list <- c(plot.list, CH4_plots)
      
      # Save to PDF using goFlux function
      if(length(plot.list) > 0) {
        flux2pdf(plot.list, outfile = "intermediate/Picarro_soilwater_CH4_flux_plots_complete.pdf", width = 12, height = 8)
        cat("Comprehensive plots saved to: intermediate/Picarro_soilwater_CH4_flux_plots_complete.pdf\n")
      }
      
      # =============================================================================
      # STEP 6G: CREATE FINAL PICARRO SOIL WATER DATASET
      # =============================================================================
      
      cat("\n=== STEP 6G: CREATING FINAL PICARRO SOIL WATER DATASET ===\n")
      
      # Load original soil water data
      original_data <- read_csv("intermediate/main_soilwater_complete.csv")
      ...
      final_dataset_picarro <- final_dataset_picarro %>%
        mutate(
          ...
          measurement_type = "soil water",
          notes = "Timestamps corrected by -25220 seconds, CH4 focus, soil water measurements"
        )
      
      # Save the final dataset
      write_csv(final_dataset_picarro, "intermediate/Picarro_soilwater_CH4_final_complete_dataset.csv")
      
      cat("Final Picarro soil water dataset saved to: intermediate/Picarro_soilwater_CH4_final_complete_dataset.csv\n")
      ...
      cat("\n=== PICARRO SOIL WATER PROCESSING COMPLETE ===\n")
      cat("Final dataset created with", nrow(final_dataset_picarro), "rows and", ncol(final_dataset_picarro), "columns\n")
      cat("Primary focus: CH4 flux measurements from soil water\n")
      cat("Timestamp correction: -25220 seconds applied\n")
      
      cat("\nFiles created:\n")
      cat("- intermediate/Picarro_soilwater_CH4_final_complete_dataset.csv (complete final dataset)\n")
      if(exists("CH4_best_picarro")) {
        cat("- intermediate/CH4_best_flux_picarro_soilwater_results.csv (CH4 flux results)\n")
      }
      if(exists("CO2_best_picarro")) {
        cat("- intermediate/CO2_best_flux_picarro_soilwater_results.csv (CO2 flux results)\n")
      }
      cat("- intermediate/Picarro_soilwater_CH4_flux_plots_complete.pdf (all plots)\n")
      ...
      cat("ERROR: No successful batches processed for Picarro soil water data\n")
    }
  }
...
cat("\n==================================================================\n")
cat("    COMPLETE SOIL WATER FLUX PROCESSING WORKFLOW FINISHED\n")
...
for(analyzer in names(analyzer_data)) {
  if(!is.null(analyzer_data[[analyzer]])) {
    if(analyzer == "Picarro") {
      cat("✓", analyzer, "- Successfully processed (CH4 focus, soil water, timestamps corrected -25220 sec)\n")
    } else {
...
cat("\nCheck the intermediate/ directory for all output files and plots.\n")
cat("Manual identification plots have been saved for quality review.\n")
cat("All Picarro output files include 'soilwater' to indicate soil water measurements.\n")
cat("\nWorkflow complete!\n")