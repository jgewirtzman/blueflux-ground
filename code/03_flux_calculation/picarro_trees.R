...
if ("Picarro" %in% names(analyzer_data) && !is.null(analyzer_data[["Picarro"]])) {
  ...
  if (length(good_windows) == 0) {
    ...
    for(batch_num in 1:num_batches) {
      ...
      tryCatch({
        manID_batch <- click.peak2(
          ...
          save.plots = paste0("intermediate/picarro_batch_", batch_num, "_plots"),
          warn.length = 60
        )
    ...
    if(length(successful_picarro_batches) > 0) {
      ...
      write_csv(manID.picarro, "intermediate/picarro_manual_identification_CH4_results.csv")
      cat("Results saved to: intermediate/picarro_manual_identification_CH4_results.csv\n")
      ...
      if(!is.null(CH4_flux_picarro)) {
        write_csv(CH4_flux_picarro, "intermediate/CH4_flux_picarro_results.csv")
        cat("Picarro CH4 flux calculation complete:", nrow(CH4_flux_picarro), "measurements\n")
      }
      
      if(!is.null(CO2_flux_picarro)) {
        write_csv(CO2_flux_picarro, "intermediate/CO2_flux_picarro_results.csv")
      ...
      if(exists("CH4_best_picarro")) {
        write_csv(CH4_best_picarro, "intermediate/CH4_best_flux_picarro_results.csv")
      }
      if(exists("CO2_best_picarro")) {
        write_csv(CO2_best_picarro, "intermediate/CO2_best_flux_picarro_results.csv")
      }
      ...
      pdf("intermediate/Picarro_CH4_flux_plots_complete.pdf", width = 12, height = 8)
      ...
      dev.off()
      cat("Comprehensive plots saved to: intermediate/Picarro_CH4_flux_plots_complete.pdf\n")
      ...
      write_csv(final_dataset_picarro, "intermediate/Picarro_CH4_final_complete_dataset.csv")
      ...
      cat("\nFiles created:\n")
      cat("- intermediate/Picarro_CH4_final_complete_dataset.csv (complete final dataset)\n")
      if(exists("CH4_best_picarro")) {
        cat("- intermediate/CH4_best_flux_picarro_results.csv (CH4 flux results)\n")
      }
      if(exists("CO2_best_picarro")) {
        cat("- intermediate/CO2_best_flux_picarro_results.csv (CO2 flux results)\n")
      }
      cat("- intermediate/Picarro_CH4_flux_plots_complete.pdf (all plots)\n")
...
flux2pdf(plot.list, outfile = "intermediate/Picarro_CH4_flux_plots_complete.pdf", width = 12, height = 8)
...
cat("Final Picarro dataset saved to: intermediate/Picarro_CH4_final_complete_dataset.csv\n")
