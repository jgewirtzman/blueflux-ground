...
soil_water_complete <- read_csv("intermediate/main_soilwater_complete.csv", show_col_types = FALSE)
...
if (!dir.exists("intermediate/auxfiles")) {
  dir.create("intermediate/auxfiles", recursive = TRUE)
}
...
write_csv(auxfile_clean, "intermediate/auxfiles/soilwater_auxfile_all_instruments.csv")
cat("Complete auxfile saved: intermediate/auxfiles/soilwater_auxfile_all_instruments.csv\n")
...
write_csv(auxfile_goflux, "intermediate/auxfiles/soilwater_auxfile_all_instruments_goflux.csv")
cat("goFlux-ready auxfile saved: intermediate/auxfiles/soilwater_auxfile_all_instruments_goflux.csv\n")
...
for (analyzer in analyzers) {
  ...
  if (nrow(analyzer_auxfile) > 0) {
    ...
    analyzer_filename_csv <- paste0("intermediate/auxfiles/soilwater_auxfile_", tolower(analyzer), ".csv")
    ...
    analyzer_filename_goflux <- paste0("intermediate/auxfiles/soilwater_auxfile_", tolower(analyzer), "_goflux.csv")
...
cat("Files created in intermediate/auxfiles/:\n")
