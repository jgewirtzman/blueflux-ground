...
# Read the tree data
tree_data <- read_csv("intermediate/main_trees_complete.csv")
...
for(instrument in instruments) {
  ...
  if(nrow(instrument_data) > 0) {
    # Save standard format to intermediate/auxfiles folder
    filename_csv <- paste0("intermediate/auxfiles/tree_auxfile_", tolower(instrument), ".csv")
    ...
    filename_goflux <- paste0("intermediate/auxfiles/tree_auxfile_", tolower(instrument), "_goflux.csv")
...
# Save the complete auxfile (all instruments combined) to intermediate/auxfiles folder
write_csv(auxfile, "intermediate/auxfiles/tree_auxfile_all_instruments.csv")
cat("Complete auxfile saved as 'intermediate/auxfiles/tree_auxfile_all_instruments.csv'\n")
...
# Save the goFlux-ready version to intermediate/auxfiles folder
...
cat("All auxfiles have been saved to the intermediate/auxfiles/ directory\n")