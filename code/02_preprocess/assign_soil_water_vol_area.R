...
soil_water_data <- read_csv("intermediate/blueflux_soilwater_filled.csv", show_col_types = FALSE)

# Read chamber dimensions specific to soil/water
chamber_dims <- read_csv("data/field_notes/dimension_csvs/soil_water_dims.csv", show_col_types = FALSE)

# Read analyzer volumes (same file as trees)
additional_vol <- read_csv("data/field_notes/dimension_csvs/additional_vol.csv", show_col_types = FALSE)
...
if (nrow(complete_data) > 0) {
  tryCatch({
    ...
    # Save plots
    ggsave("intermediate/soilwater_volume_by_chamber_analyzer.png", p1, width = 12, height = 8, dpi = 300)
    ggsave("intermediate/soilwater_surface_area_by_chamber.png", p2, width = 10, height = 6, dpi = 300)
    ggsave("intermediate/soilwater_volume_components.png", p3, width = 14, height = 10, dpi = 300)
    
    cat("Plots saved:\n")
    cat("- intermediate/soilwater_volume_by_chamber_analyzer.png\n")
    cat("- intermediate/soilwater_surface_area_by_chamber.png\n")
    cat("- intermediate/soilwater_volume_components.png\n")
...
write_csv(soil_water_complete, "intermediate/main_soilwater_complete.csv")

cat("Results saved to: intermediate/main_soilwater_complete.csv\n")
