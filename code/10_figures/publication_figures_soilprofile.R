...
(ghost)")) +
  ...
  # Alternate version with legend outside (right) and slightly wider canvas
  fig5_extlegend <- fig5 +
  ...
  save_pub(fig5_extlegend, "soil_pca_extlegend", width = 250, height = 170)
...
# =============================================================================
# SI FIGURE: Per-site slopes for TA vs DIC (annotated with regression results)
# Same data as above, with slope/intercept/R2 text labels.
# =============================================================================
cat("\n--- SI Figure: TA vs DIC with slope annotations ---\n")

slope_stats <- df_nosurface %>%
  filter(!is.na(DIC_uM), !is.na(Alkalinity_uM)) %>%
  group_by(Site) %>%
  summarise(
    slope = coef(lm(Alkalinity_uM ~ DIC_uM))[2],
    intercept = coef(lm(Alkalinity_uM ~ DIC_uM))[1],
    r2 = summary(lm(Alkalinity_uM ~ DIC_uM))$r.squared,
    n = n(),
    .groups = "drop"
  ) %>%
  arrange(Site) %>%
  mutate(
    label = sprintf("%s: slope=%.2f, R^2==%.2f, n=%d",
                    Site, slope, r2, n),
    y_npc = seq(0.98, by = -0.06, length.out = n())
  )

fig_ta_dic_slopes <- df_nosurface %>%
  filter(!is.na(DIC_uM), !is.na(Alkalinity_uM)) %>%
  ggplot(aes(x = DIC_uM, y = Alkalinity_uM, color = Site, shape = Site)) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed",
              color = "grey50", linewidth = 0.5) +
  geom_smooth(aes(group = Site), method = "lm", se = FALSE,
              linewidth = 0.5, alpha = 0.6) +
  geom_point(size = 3.5) +
  geom_text(data = slope_stats,
            aes(x = -Inf, y = Inf, label = label, color = Site),
            inherit.aes = FALSE, parse = FALSE,
            hjust = -0.05, vjust = c(1.2, 2.5, 3.8, 5.1), size = 3) +
  scale_color_manual(values = site_colors, name = "Site",
                     labels = c("SRS5 (healthy)", "SRS6 (healthy)",
                                "BL60 (regen.)", "CP40 (ghost)")) +
  scale_shape_manual(values = site_shapes, name = "Site",
...
save_pub(fig_ta_dic_slopes, "SI_ta_vs_dic_slopes", width = 160, height = 110)


# =============================================================================
# SI FIGURE: TA and DIC vs salinity (conservative-mixing reference)
# ---------------------------------------------------------------------------
# MRV framing: conservative mixing of marine and freshwater endmembers gives
# a reference line for "no in-system alkalinity production." Excess (above
# line) = mangrove-generated alkalinity. The slope/offset of the cloud
# relative to the reference informs lateral export potential and drives the
# baseline-vs-signal question.
#
# Florida endmembers (representative):
#   S=0  : Everglades freshwater TA ~3000 uM (limestone-influenced)
#   S=35 : Florida coastal seawater TA ~2400 uM
# These are conservative defaults; real endmembers vary seasonally and
# spatially -- the line is a heuristic, not a calibrated model.
# =============================================================================
cat("\n--- SI Figures: TA / DIC vs salinity ---\n")

TA_fw <- 3000   # uM
TA_sw <- 2400   # uM
DIC_fw <- 2800  # uM (slightly less than TA at FW endmember)
DIC_sw <- 2100  # uM

mix_line <- function(S, c_fw, c_sw) c_fw + (c_sw - c_fw) * S / 35

mix_df <- tibble::tibble(
  PSU = seq(0, 60, length.out = 100),
  TA_mix  = mix_line(PSU, TA_fw, TA_sw),
  DIC_mix = mix_line(PSU, DIC_fw, DIC_sw)
)

# --- TA vs salinity ---
ta_sal_stats <- df_nosurface %>%
  filter(!is.na(PSU), !is.na(Alkalinity_uM)) %>%
  group_by(disturbance) %>%
  summarise(
    n = n(),
    r = cor(PSU, Alkalinity_uM),
    p_val = cor.test(PSU, Alkalinity_uM)$p.value,
    .groups = "drop"
  ) %>%
  mutate(label = sprintf("italic(r) == %.2f*','~italic(n) == %d", r, n))

fig_ta_sal <- df_nosurface %>%
  filter(!is.na(PSU), !is.na(Alkalinity_uM)) %>%
  ggplot(aes(x = PSU, y = Alkalinity_uM,
             color = disturbance, shape = Site)) +
  geom_line(data = mix_df, aes(x = PSU, y = TA_mix),
            inherit.aes = FALSE, linetype = "dashed",
            color = "grey50", linewidth = 0.6) +
  annotate("text", x = 58, y = mix_line(58, TA_fw, TA_sw),
           label = "conservative\nmixing", hjust = 1, vjust = -0.3,
           size = 2.8, color = "grey40") +
  geom_smooth(aes(group = disturbance, fill = disturbance),
              method = "lm", se = TRUE, alpha = 0.15, linewidth = 0.7) +
  geom_point(size = 3.5, alpha = 0.85) +
  geom_text(data = ta_sal_stats,
            aes(x = Inf, y = Inf, label = label, color = disturbance),
            inherit.aes = FALSE, parse = TRUE,
            hjust = 1.05,
            vjust = seq(1.2, by = 1.4, length.out = nrow(ta_sal_stats)),
            size = 3) +
...
save_pub(fig_ta_sal, "SI_ta_vs_salinity", width = 160, height = 110)

# --- DIC vs salinity ---
dic_sal_stats <- df_nosurface %>%
  filter(!is.na(PSU), !is.na(DIC_uM)) %>%
  group_by(disturbance) %>%
  summarise(
    n = n(),
    r = cor(PSU, DIC_uM),
...
fig_dic_sal <- df_nosurface %>%
  filter(!is.na(PSU), !is.na(DIC_uM)) %>%
  ggplot(aes(x = PSU, y = DIC_uM,
             color = disturbance, shape = Site)) +
  geom_line(data = mix_df, aes(x = PSU, y = DIC_mix),
            inherit.aes = FALSE, linetype = "dashed",
            color = "grey50", linewidth = 0.6) +
  annotate("text", x = 58, y = mix_line(58, DIC_fw, DIC_sw),
           label = "conservative\nmixing", hjust = 1, vjust = -0.3,
           size = 2.8, color = "grey40") +
  geom_smooth(aes(group = disturbance, fill = disturbance),
              method = "lm", se = TRUE, alpha = 0.15, linewidth = 0.7) +
  geom_point(size = 3.5, alpha = 0.85) +
  geom_text(data = dic_sal_stats,
            aes(x = Inf, y = Inf, label = label, color = disturbance),
            inherit.aes = FALSE, parse = TRUE,
            hjust = 1.05,
            vjust = seq(1.2, by = 1.4, length.out = nrow(dic_sal_stats)),
            size = 3) +
  scale_color_manual(values = disturbance_colors, name = "Disturbance") +
  scale_fill_manual(values = disturbance_colors, guide = "none") +
  scale_shape_manual(values = site_shapes, name = "Site") +
...
save_pub(fig_dic_sal, "SI_dic_vs_salinity", width = 160, height = 110)


# =============================================================================
# SI FIGURE: salinity vs TAlk (style-matched to pub_SI_salinity_vs_ch4)
# Single campaign (Oct 2025), so no season dimension. Disturbance-colored
# with per-class linear fit and Pearson r / n labels in the top right.
# =============================================================================
cat("\n--- SI Figure: salinity vs TAlk (CH4-style) ---\n")

talk_stats <- df_nosurface %>%
  filter(!is.na(PSU), !is.na(Alkalinity_uM)) %>%
  group_by(disturbance) %>%
  summarise(
    n = n(),
    r = cor(PSU, Alkalinity_uM, use = "complete.obs"),
    p_val = cor.test(PSU, Alkalinity_uM)$p.value,
    .groups = "drop"
  ) %>%
  mutate(
...
p_scatter_talk <- df_nosurface %>%
  filter(!is.na(PSU), !is.na(Alkalinity_uM)) %>%
  ggplot(aes(x = PSU, y = Alkalinity_uM, color = disturbance)) +
  geom_point(size = 3, alpha = 0.85) +
  geom_smooth(aes(group = disturbance, fill = disturbance),
              method = "lm", se = TRUE, alpha = 0.15, linewidth = 0.7) +
  scale_color_manual(values = disturbance_colors, name = "Disturbance") +
  scale_fill_manual(values = disturbance_colors, guide = "none") +
  geom_text(data = talk_stats,
            aes(x = Inf, y = Inf, label = label, color = disturbance),
            inherit.aes = FALSE, parse = TRUE,
            hjust = 1.05,
            vjust = seq(1.2, by = 1.3, length.out = nrow(talk_stats)),
            size = 3) +
  labs(x = "Salinity (PSU)",
       y = expression(Total~Alkalinity~(mu*M))) +
  theme_pub(base_size = 11)

save_pub(p_scatter_talk, "SI_salinity_vs_TAlk", width = 8 * 25.4, height = 6 * 25.4)


# =============================================================================
# SI FIGURE: Excess TA / Excess DIC vs salinity (mangrove-attributable signal)
# Subtract conservative-mixing reference from observed concentrations.
# Vertical position above zero = mangrove-generated TA / DIC.
# Slope of excess-TA vs excess-DIC is the pathway diagnostic with the
# saltwater-mixing dilution removed (~1 = SR; >1 = carbonate dissolution;
# <1 = aerobic / methanotrophic addition of DIC without TA).
# =============================================================================
cat("\n--- SI Figure: Excess TA & DIC (mangrove-attributable) ---\n")

df_excess <- df_nosurface %>%
  filter(!is.na(PSU), !is.na(Alkalinity_uM), !is.na(DIC_uM)) %>%
  mutate(
    TA_excess  = Alkalinity_uM - mix_line(PSU, TA_fw, TA_sw),
    DIC_excess = DIC_uM        - mix_line(PSU, DIC_fw, DIC_sw)
  )

# Panel A: TA_excess vs salinity
fig_taex_sal <- df_excess %>%
  ggplot(aes(x = PSU, y = TA_excess,
             color = disturbance, shape = Site)) +
  ...
  scale_shape_manual(values = site_shapes, name = "Site") +
  labs(x = "Salinity (PSU)",
       y = expression(Excess~TA~(mu*M)),
       subtitle = "TA above (or below) conservative seawater–freshwater mixing") +
  theme_pub()

# Panel B: TA_excess vs DIC_excess (Deffeyes-style with 1:1 reference)
ex_slopes <- df_excess %>%
  group_by(Site) %>%
  summarise(slope = coef(lm(TA_excess ~ DIC_excess))[2],
            r2 = summary(lm(TA_excess ~ DIC_excess))$r.squared,
            n = n(), .groups = "drop") %>%
  arrange(Site) %>%
  mutate(label = sprintf("%s: slope=%.2f, R^2==%.2f", Site, slope, r2))

fig_taex_dicex <- df_excess %>%
  ggplot(aes(x = DIC_excess, y = TA_excess,
  ...
  scale_shape_manual(values = site_shapes, name = "Site",
                     labels = c("SRS5 (healthy)", "SRS6 (healthy)",
                                "BL60 (regen.)", "CP40 (ghost)")) +
  labs(x = expression(Excess~DIC~(mu*M)),
       y = expression(Excess~TA~(mu*M)),
       subtitle = "In-system TA generation per unit DIC, mixing removed") +
  theme_pub()

fig_excess <- fig_taex_sal + fig_taex_dicex +
  plot_annotation(tag_levels = "a") +
  plot_layout(widths = c(1, 1.1))

save_pub(fig_excess, "SI_excess_TA_DIC", width = 240, height = 110)


# =============================================================================
# SI FIGURE: TA-excess vs SO4 deficit (sulfate-reduction stoichiometry)
# Classical SR yields ~2 mol HCO3- per mol SO4 reduced (slope = 2).
# Plotting excess-TA (mixing-corrected) against SO4-deficit (relative to
# conservative seawater scaling, 2712 mg/L * S/35) tests whether the
# alkalinity factory is biogenic SR (slope ~2) vs carbonate dissolution
# (slope >> 2; SO4 not consumed) vs evaporite re-supply (slope < 2; SO4
# replenished by gypsum, masking SR signal -- relevant at hypersaline CP40).
# =============================================================================
cat("\n--- SI Figure: TA-excess vs SO4 deficit ---\n")

df_so4 <- df_nosurface %>%
  filter(!is.na(PSU), !is.na(Alkalinity_uM), !is.na(SO4_ppm)) %>%
  mutate(
    SO4_uM_obs = SO4_ppm / 96.06 * 1000,                       # mg/L -> uM
    SO4_uM_expected = (2712 * PSU / 35) / 96.06 * 1000,
    SO4_deficit_uM = SO4_uM_expected - SO4_uM_obs,
    TA_excess = Alkalinity_uM - mix_line(PSU, TA_fw, TA_sw)
  )

# Reference lines: pure SR (slope 2), and slope 1 for comparison
ref_lines <- tibble::tibble(
  slope = c(2, 1),
  label = c("2:1 (sulfate reduction)", "1:1")
)

so4_stats <- df_so4 %>%
  group_by(Site) %>%
  summarise(
    slope = coef(lm(TA_excess ~ SO4_deficit_uM))[2],
    r2 = summary(lm(TA_excess ~ SO4_deficit_uM))$r.squared,
    n = n(), .groups = "drop"
  ) %>%
  arrange(Site) %>%
  mutate(label = sprintf("%s: slope=%.2f, R^2==%.2f", Site, slope, r2))
...
fig_so4 <- df_so4 %>%
  ggplot(aes(x = SO4_deficit_uM, y = TA_excess,
             color = Site, shape = Site)) +
  ...
  annotate("text", x = Inf, y = Inf,
           label = "2:1 = sulfate reduction",
           hjust = 1.05, vjust = 1.5, size = 3, color = "darkred") +
  geom_smooth(aes(group = Site), method = "lm", se = FALSE,
              linewidth = 0.5, alpha = 0.6) +
  geom_point(size = 3.5) +
  geom_text(data = so4_stats,
            aes(x = -Inf, y = Inf, label = label, color = Site),
            inherit.aes = FALSE, parse = FALSE,
            hjust = -0.05, vjust = c(2.8, 4.1, 5.4, 6.7), size = 3) +
  scale_color_manual(values = site_colors, name = "Site",
                     labels = c("SRS5 (healthy)", "SRS6 (healthy)",
                                "BL60 (regen.)", "CP40 (ghost)")) +
  scale_shape_manual(values = site_shapes, name = "Site",
                     labels = c("SRS5 (healthy)", "SRS6 (healthy)",
                                "BL60 (regen.)", "CP40 (ghost)")) +
  labs(x = expression(SO[4]^{"2-"}~deficit~(mu*M)),
       y = expression(Excess~TA~(mu*M)),
       subtitle = "Stoichiometric test: biogenic SR predicts slope = 2") +
  theme_pub()

save_pub(fig_so4, "SI_TA_vs_SO4_deficit", width = 160, height = 120)

# Clean version: no slope/R^2 text labels and no SR reference annotation
fig_so4_clean <- df_so4 %>%
  ggplot(aes(x = SO4_deficit_uM, y = TA_excess,
             color = Site, shape = Site)) +
  geom_abline(slope = 2, intercept = 0, linetype = "dashed",
              color = "darkred", linewidth = 0.5) +
...
save_pub(fig_so4_clean, "SI_TA_vs_SO4_deficit_clean", width = 160, height = 120)


# =============================================================================
# SI FIGURE: Climate ledger -- CH4 (warming) vs TA-driven CO2 sequestration
# (cooling), as CO2-eq per site. Two opposing bars per site; net annotated.
# Honest framing:
#   - CH4 efflux (chamber medians) -> CO2-eq via GWP100 = 27 (AR6)
#   - TA-driven CO2 uptake = TA flux x 0.85 mol CO2 per mol TA-eq x 44 g/mol
#     (Renforth & Henderson 2017 ocean uptake efficiency). TA flux uses
#     Fick's law diffusive lower bound from porewater gradient (5 cm to
#     freshwater-endmember TA = 3000 uM, phi=0.7, D=1e-4 m^2/d, theta^2=1.7).
#   - Direct soil CO2 efflux NOT included: it is part of a coupled
#     NPP-respiration cycle and requires NEP context to interpret as a
#     climate signal. CH4 and TA are both unambiguously net flows in/out.
# =============================================================================
cat("\n--- SI Figure: Climate-equivalent ledger (CH4 warming vs TA cooling) ---\n")

surf_flux <- read_csv("output/data_products/soil_water_surface_fluxes.csv",
                      show_col_types = FALSE) %>%
  filter(plot %in% c("SRS5", "SRS6", "BL60", "CP40")) %>%
  group_by(plot) %>%
  summarise(
    CH4_med_nmol_m2_s = median(CH4_best.flux, na.rm = TRUE),
    CO2_med_umol_m2_s = median(CO2_best.flux, na.rm = TRUE),
    n = n(),
    .groups = "drop"
  ) %>%
  mutate(
    Site = factor(plot, levels = c("SRS5", "SRS6", "BL60", "CP40")),
    disturbance = case_when(
      plot %in% c("SRS5", "SRS6") ~ "healthy",
      plot == "BL60" ~ "regenerating",
      plot == "CP40" ~ "ghost"
    ) %>% factor(levels = c("healthy", "regenerating", "ghost")),
    # CH4 emission as CO2-eq:
    # nmol/m2/s * 86400 * 365 / 1e9 mol/m2/yr * 16.04 g/mol = g CH4/m2/yr
    # * 27 (GWP100) = g CO2-eq/m2/yr
    CH4_warming_gCO2eq_m2_yr = CH4_med_nmol_m2_s * 86400 * 365 / 1e9 * 16.04 * 27
  )

# Per-site diffusive TA flux estimates (mid-range from porewater gradients;
# computed in Q&A above). Replace with computed values from the porewater
# gradient if a closed-form calc is added later.
ta_flux_lookup <- tibble::tibble(
  Site = factor(c("SRS5", "SRS6", "BL60", "CP40"),
                levels = c("SRS5", "SRS6", "BL60", "CP40")),
  TA_flux_mmol_m2_d = c(3, 3, 17, 10)
) %>%
  mutate(
    # mmol/m2/d * 365 d/yr / 1000 = mol/m2/yr
    # * 0.85 mol CO2 per mol TA-eq * 44.01 g/mol CO2
    TA_cooling_gCO2_m2_yr = TA_flux_mmol_m2_d * 365 / 1000 * 0.85 * 44.01
  )

ledger <- surf_flux %>% left_join(ta_flux_lookup, by = "Site") %>%
  # Net forcing in plot's sign convention: positive y-axis = warming,
  # negative y-axis = cooling. So net = CH4 warming - TA cooling.
  mutate(net_gCO2eq_m2_yr = CH4_warming_gCO2eq_m2_yr - TA_cooling_gCO2_m2_yr)

ledger_long <- ledger %>%
  select(Site, disturbance,
         `CH4 emission (CO2-eq)` = CH4_warming_gCO2eq_m2_yr,
         `TA-driven CO2 uptake`   = TA_cooling_gCO2_m2_yr) %>%
  tidyr::pivot_longer(c(`CH4 emission (CO2-eq)`,
                        `TA-driven CO2 uptake`),
                      names_to = "term", values_to = "value") %>%
  mutate(value = ifelse(term == "TA-driven CO2 uptake", -value, value))

fig_ledger <- ggplot(ledger_long,
                     aes(x = Site, y = value, fill = term)) +
  geom_col(width = 0.7, color = "black", linewidth = 0.3) +
  geom_hline(yintercept = 0, linewidth = 0.5) +
  geom_point(data = ledger,
             aes(x = Site, y = net_gCO2eq_m2_yr),
             inherit.aes = FALSE, shape = 23, fill = "white",
             color = "black", size = 3.5, stroke = 1) +
  geom_text(data = ledger,
            aes(x = Site, y = net_gCO2eq_m2_yr,
                label = sprintf("net %+0.0f", net_gCO2eq_m2_yr)),
            inherit.aes = FALSE,
            vjust = -1.2, size = 3) +
  scale_fill_manual(values = c("CH4 emission (CO2-eq)" = "#B22222",
                               "TA-driven CO2 uptake"  = "#1F77B4"),
                    name = NULL) +
  labs(x = NULL,
       y = expression(g~CO[2]*"-eq"~m^{-2}~yr^{-1}),
       subtitle = paste("Warming (above 0) vs cooling (below 0).",
                        "TA flux is diffusive lower bound."),
       caption = paste("CH4 GWP100 = 27 (AR6); TA->CO2 sequestration",
                       "efficiency = 0.85 mol CO2/mol TA-eq. Soil CO2",
                       "efflux excluded (coupled to NPP).")) +
  theme_pub() +
  theme(legend.position = "top",
        plot.caption = element_text(size = 8, color = "grey40"))

save_pub(fig_ledger, "SI_climate_ledger", width = 180, height = 130)
...
if (!is.null(fig8_left)) {
  ...
  source("code/10_figures/site_characterization_figures.R")
...
# DONE
# =============================================================================
cat("\n===== All soil profile figures saved to output/figures/other/ =====\n")
