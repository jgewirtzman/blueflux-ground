...
ggsave("output/figures/other/pub_SI_depth_profiles_lines.pdf", layout1, width = 12, height = 7)
ggsave("output/figures/other/pub_SI_depth_profiles_lines.png", layout1, width = 12, height = 7, dpi = 300)
cat("  Saved layout 1\n")
...
ggsave("output/figures/other/pub_SI_depth_profiles_grid.pdf", p2, width = 7, height = 10)
ggsave("output/figures/other/pub_SI_depth_profiles_grid.png", p2, width = 7, height = 10, dpi = 300)
cat("  Saved layout 2\n")
...
ggsave("output/figures/other/pub_SI_depth_profiles_heatmap.pdf", layout3, width = 14, height = 8)
ggsave("output/figures/other/pub_SI_depth_profiles_heatmap.png", layout3, width = 14, height = 8, dpi = 300)
cat("  Saved layout 3\n")
...
ggsave("output/figures/other/pub_SI_depth_profiles_bubbles.pdf", layout4, width = 12, height = 8)
ggsave("output/figures/other/pub_SI_depth_profiles_bubbles.png", layout4, width = 12, height = 8, dpi = 300)
cat("  Saved layout 4\n")
...
ggsave("output/figures/other/pub_SI_salinity_vs_ch4.pdf", p_scatter, width = 8, height = 6)
ggsave("output/figures/other/pub_SI_salinity_vs_ch4.png", p_scatter, width = 8, height = 6, dpi = 300)
cat("  Saved salinity vs CH4 scatter\n")

# Blue-shades variant
disturbance_blues <- c("healthy" = "#08306B", "regenerating" = "#4292C6", "ghost" = "#9ECAE1")

p_scatter_blue <- merged_complete %>%
  ggplot(aes(x = PSU_mean, y = CH4_mean, color = disturbance, shape = season)) +
  geom_point(size = 3, alpha = 0.85) +
  geom_smooth(aes(group = disturbance, fill = disturbance),
              method = "lm", se = TRUE, alpha = 0.15, linewidth = 0.7) +
  scale_y_continuous(trans = "log1p", breaks = c(0, 1, 5, 10, 25, 50, 100)) +
  scale_color_manual(values = disturbance_blues, name = "Disturbance") +
  scale_fill_manual(values = disturbance_blues, guide = "none") +
  scale_shape_manual(values = c(16, 17, 15), name = "Campaign") +
  geom_text(data = scatter_stats,
            aes(x = Inf, y = Inf, label = label, color = disturbance),
            inherit.aes = FALSE, parse = TRUE,
...
ggsave("output/figures/other/pub_SI_salinity_vs_ch4_blue.pdf", p_scatter_blue, width = 8, height = 6)
...
ggsave("output/figures/other/pub_SI_porewater_characterization.pdf", combined_si,
       width = 12, height = 14)
ggsave("output/figures/other/pub_SI_porewater_characterization.png", combined_si,
