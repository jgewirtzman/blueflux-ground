...
lgr3_auxfile <- read_csv("intermediate/auxfiles/lgr3_rescue_auxfile.csv") %>% mutate(start.time = as.POSIXct(start.time, tz = ifelse(lgr3_data_tz == "", "UTC", lgr3_data_tz)))
...
good_windows <- which(window_sizes >= 10)
...
# Save flux results
write_csv(CO2_flux_lgr3, "intermediate/CO2_flux_lgr3_results_rescue.csv")
if(exists("CH4_flux_lgr3")) {
  write_csv(CH4_flux_lgr3, "intermediate/CH4_flux_lgr3_results_rescue.csv")
}
...
write_csv(CO2_best_lgr3, "intermediate/CO2_best_flux_lgr3_results_rescue.csv")
if(exists("CH4_best_lgr3")) {
  write_csv(CH4_best_lgr3, "intermediate/CH4_best_flux_lgr3_results_rescue.csv")
}
...
flux2pdf(
  ...
  outfile = "intermediate/LGR3_flux_plots_complete_rescue.pdf",
  width = 11.6,
  height = 8.2
)
