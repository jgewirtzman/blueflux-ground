...
corrected_measurement <- click.peak2(
  ...
  plot.lim = c(200, 1000),
  save.plots = paste0("intermediate/corrected_", problematic_id, "_plot")
)
...
if(length(rows_to_update) > 0) {
  ...
  write_csv(manID.lgr1, "intermediate/lgr1_manual_identification_results_updated.csv")
  cat("Updated results saved to: intermediate/lgr1_manual_identification_results_updated.csv\n")
...
write_csv(CO2_best_lgr1_updated, "intermediate/CO2_best_flux_lgr1_results_updated.csv")
...
flux2pdf(
  ...
  outfile = paste0("intermediate/", problematic_id, "_corrected_plot.pdf"),
  width = 11.6,
  height = 8.2
)
...
cat("\nFiles created:\n")
cat("- intermediate/lgr1_manual_identification_results_updated.csv\n")
cat("- intermediate/CO2_best_flux_lgr1_results_updated.csv\n")
cat("- intermediate/", problematic_id, "_corrected_plot.pdf\n")

cat("\nTo use updated results going forward:\n")
cat("manID.lgr1 <- read_csv('intermediate/lgr1_manual_identification_results_updated.csv')\n")
cat("CO2_best_lgr1 <- read_csv('intermediate/CO2_best_flux_lgr1_results_updated.csv')\n")
