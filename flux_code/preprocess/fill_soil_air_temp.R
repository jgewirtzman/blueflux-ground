...
soil_water_data <- read_csv("field_notes/BlueFlux Dataset_soils_water.csv", show_col_types = FALSE)
...
tree_files <- c(
  "field_notes/blueflux compiled tree fluxes.csv",
  "field_notes/blueflux compiled tree fluxes_additional.csv"
)
...
create_temp_source_plot <- function(soil_data, tree_data, weather_data = NULL) {
  ...
  if (nrow(plot_data) > 0) {
    ...
    # Save both plots
    ggsave("flux_code/air_temperature_sources_overview.png", p1, 
           width = 14, height = 10, dpi = 300)
    ggsave("flux_code/air_temperature_filling_methods.png", p2, 
           width = 16, height = 12, dpi = 300)
    
    cat("Temperature plots saved:\n")
    cat("- flux_code/air_temperature_sources_overview.png (overview)\n")
    cat("- flux_code/air_temperature_filling_methods.png (detailed filling methods)\n")
...
write_csv(soil_water_filled, "flux_code/intermediate_files/blueflux_soilwater_filled.csv")

cat("Filled soil/water data saved to: flux_code/intermediate_files/blueflux_soilwater_filled.csv\n")
