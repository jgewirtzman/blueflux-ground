...
# Set data path
data_path <- "analyzer_data/LGR_GLA131/LGR2"
...
# Load LGR2 auxfile
lgr2_auxfile <- read_csv("flux_code/auxfiles/lgr2_rescue_auxfile.csv") %>%
...
for(batch_num in 1:num_batches) {
  ...
  manID_batch <- click.peak2(
    ...
    sleep = 3,
    plot.lim = c(200, 1600),
    save.plots = paste0("flux_code/lgr2_batch_", batch_num, "_plots")
  )
...
write_csv(manID.lgr2, "flux_code/lgr2_manual_identification_results.csv")
...
# Save flux results
write_csv(CO2_flux_lgr2, "flux_code/CO2_flux_lgr2_rescue.csv")
if(exists("CH4_flux_lgr2")) {
  write_csv(CH4_flux_lgr2, "flux_code/CH4_flux_lgr2_rescue.csv")
}
...
write_csv(CO2_best_lgr2, "flux_code/CO2_best_flux_lgr2_rescue.csv")
if(exists("CH4_best_lgr2")) {
  write_csv(CH4_best_lgr2, "flux_code/CH4_best_flux_lgr2_rescue.csv")
}
...
flux2pdf(
  ...
  outfile = "flux_code/LGR2_flux_plots_rescue.pdf",
  width = 11.6,
  height = 8.2
)
...
flux2pdf(
  ...
  outfile = "flux_code/LGR2_CO2_flux_plots.pdf",
  width = 11.6,
  height = 8.2
)
...
if(exists("CH4_plots_lgr2")) {
  flux2pdf(
    ...
    outfile = "flux_code/LGR2_CH4_flux_plots.pdf",
    width = 11.6,
    height = 8.2
  )
}
...
original_data <- read_csv("combined_gas_flux_dataset_with_month_year.csv")
...
write_csv(final_dataset, "final_complete_dataset_rescue.csv")
...
cat("\nFiles created:\n")
cat("- flux_code/LGR2_final_complete_dataset.csv (complete final dataset)\n")
cat("- flux_code/CO2_best_flux_lgr2_results.csv (CO2 flux results)\n")
if(exists("CH4_best_lgr2")) {
  cat("- flux_code/CH4_best_flux_lgr2_results.csv (CH4 flux results)\n")
}
cat("- flux_code/LGR2_flux_plots_complete.pdf (all plots)\n")
