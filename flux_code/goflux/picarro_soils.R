...
if ("Picarro" %in% names(analyzer_data) && !is.null(analyzer_data[["Picarro"]])) {
  ...
  picarro_auxfile_path <- "flux_code/auxfiles/soilwater_auxfile_picarro_goflux.csv"
  ...
  if (length(good_windows) == 0) {
    ...
    for(batch_num in 1:num_batches) {
      ...
      tryCatch({
        manID_batch <- click.peak2(
          ...
          save.plots = paste0("flux_code/picarro_soilwater_batch_", batch_num, "_plots"),
          warn.length = 60
        )
    ...
    if(length(successful_picarro_batches) > 0) {
      ...
      write_csv(manID.picarro, "flux_code/picarro_soilwater_manual_identification_CH4_results.csv")
      cat("Results saved to: flux_code/picarro_soilwater_manual_identification_CH4_results.csv\n")
      ...
      if(!is.null(CH4_flux_picarro)) {
        write_csv(CH4_flux_picarro, "flux_code/CH4_flux_picarro_soilwater_results.csv")
        cat("Picarro soil water CH4 flux calculation complete:", nrow(CH4_flux_picarro), "measurements\n")
      }
      
      if(!is.null(CO2_flux_picarro)) {
        write_csv(CO2_flux_picarro, "flux_code/CO2_flux_picarro_soilwater_results.csv")
      ...
      if(exists("CH4_best_picarro")) {
        write_csv(CH4_best_picarro, "flux_code/CH4_best_flux_picarro_soilwater_results.csv")
      }
      if(exists("CO2_best_picarro")) {
        write_csv(CO2_best_picarro, "flux_code/CO2_best_flux_picarro_soilwater_results.csv")
      }
      ...
      if(length(plot.list) > 0) {
        flux2pdf(plot.list, outfile = "flux_code/Picarro_soilwater_CH4_flux_plots_complete.pdf", width = 12, height = 8)
        cat("Comprehensive plots saved to: flux_code/Picarro_soilwater_CH4_flux_plots_complete.pdf\n")
      }
      ...
      original_data <- read_csv("flux_code/intermediate_files/main_soilwater_complete.csv")
      ...
      write_csv(final_dataset_picarro, "flux_code/Picarro_soilwater_CH4_final_complete_dataset.csv")
      
      cat("Final Picarro soil water dataset saved to: flux_code/Picarro_soilwater_CH4_final_complete_dataset.csv\n")
      ...
      cat("\nFiles created:\n")
      cat("- flux_code/Picarro_soilwater_CH4_final_complete_dataset.csv (complete final dataset)\n")
      if(exists("CH4_best_picarro")) {
        cat("- flux_code/CH4_best_flux_picarro_soilwater_results.csv (CH4 flux results)\n")
      }
      if(exists("CO2_best_picarro")) {
        cat("- flux_code/CO2_best_flux_picarro_soilwater_results.csv (CO2 flux results)\n")
      }
      cat("- flux_code/Picarro_soilwater_CH4_flux_plots_complete.pdf (all plots)\n")
