...
picarro_data <- import2RData(
  path = "analyzer_data/Picarro_G4301",
  instrument = "G4301",
  date.format = "mdy",
  timezone = "UTC",
  keep_all = FALSE,
...
# Step 2: Load auxfile
cat("Loading auxfile...\n")
picarro_auxfile <- read_csv("flux_code/auxfiles/picarro_rescue_auxfile.csv") %>%
...
# Step 8: Save results
write_csv(CO2_best_picarro, "flux_code/rescue_CO2_picarro_results.csv")
if(!is.null(CH4_best_picarro)) {
  write_csv(CH4_best_picarro, "flux_code/rescue_CH4_picarro_results.csv")
}
...
cat("\nFiles saved:\n")
cat("- flux_code/rescue_CO2_picarro_results.csv\n")
if(!is.null(CH4_best_picarro)) {
  cat("- flux_code/rescue_CH4_picarro_results.csv\n")
}
